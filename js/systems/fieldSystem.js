...
export function createFieldForSubject(state, subject, cropId = "wheat") {
  const field = {
    id: uid("field"),
...
export function updateFieldProgress(state, subjectId) {
  ...
  field.weedCount = state.logs
    .filter(log => log.subjectId === subjectId)
    .reduce((sum, log) => sum + (log.weeds?.length || 0), 0);
  ...
  field.harvestReady =
    field.growth >= 100 &&
    tasks.length > 0 &&
    field.harvestedCount === 0;
}

export function updateAllFields(state) {
  state.fields.forEach(field => updateFieldProgress(state, field.subjectId));
}

export function harvestField(state, fieldId) {
  const field = state.fields.find(field => field.id === fieldId);

  if (!field || !field.harvestReady || field.harvestedCount > 0) {
    return false;
  }
